...
import {  removeItem } from "../redux/actions/productsActions";
const Cart= ({show, setShow}) =>{
    const carts = useSelector(state => state.cart);
    console.log(carts)
    const renderCart = carts.length>0 && carts?.map((cart,i) => {
        const { id, title, price, image, category } = cart
        console.log(cart)
        return (
            <>
    ...
    return (
        <>
            <div>
                {show && (
                    <div className="w-full h-full bg-black bg-opacity-90 top-0 overflow-y-auto overflow-x-hidden fixed sticky-0 z-20" id="chec-div">
                                ...
                                <div className=" md:w-1/3 xl:w-1/4 w-full bg-gray-100 h-full">
                                    <div className="flex flex-col md:h-screen px-14 py-20 justify-between overflow-y-auto">
                                        ...
                                        <div>
                                            ...
                                            <Link  to="/checkout"  onClick={() => setShow(!show)} style={{width:'100%',padding:'8px',borderRadius:'5px'}} className="text-base leading-none w-full py-5 bg-gray-800 border-gray-800 border focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-800 text-white">
                                                Checkout
                                            </Link >
                                        </div>
                                    </div>
